...
public class LowCortisolBowItem extends BowItem {
    public LowCortisolBowItem(Properties pProperties) {
        super(pProperties);
    }

    public static float getDamageForCortisol(float cortisol) {
        if (cortisol <= 30f) return 3f;
        if (cortisol <= 60f) return 2f;
        if (cortisol <= 80f) return 1f;
        if (cortisol <= 100f) return 0f;
            ...
            if (!itemstack.isEmpty() || flag) {
                ...
                if (!((double)f < 0.1D)) {
                    ...
                    if (!pLevel.isClientSide) {
                        ...
                        AbstractArrow temparrow = arrowitem.createArrow(pLevel, itemstack, player);
                        final AbstractArrow abstractarrow = customArrow(temparrow);
                        abstractarrow.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, f * 3.0F, 1.0F);

                        player.getCapability(PlayerCortisolProvider.PLAYER_CORTISOL).ifPresent(cortisol -> {


                            float damage = getDamageForCortisol(cortisol.getCortisol());
